...
[["text" => "- To Account"],["text" =>"- Clear list"]],
[["text" =>"- Delet Number"],["text" =>"- Add Number"]]], ])
])->result->message_id;
bot('deleteMessage',[
'chat_id'=>$chat_id,
'message_id'=>$message_id + 1,
]);
bot('sendMessage', ['chat_id' => $chat_id, 'text' => "Welcome , Developer ",
]);
}
if($text == "sstates") {
$a = states("a");
...
if ($text == "->") {
bot('sendMessage', ['chat_id' => $chat_id, 'text' => "Welcome , Developer ", 'reply_markup' => json_encode(['resize_keyboard' => true, 'keyboard' => [
[["text" =>"- Requests"],["text" =>"- Clear All list"]],
[["text" =>"- Run Turbo"],["text" =>"- Stop Turbo"]],
[["text" =>"- Add Users"],["text" =>"- Delet User"]],
[["text" => "- To Account"],["text" =>"- Clear list"]],
[["text" =>"- Delet Number"],["text" =>"- Add Number"]]], ]) ]);
}
...
if ($text == "- Clear All list") {
    $clearlist = file_get_contents("users");
    $clearlist2 = file_get_contents("u2");
    $clearlist3 = file_get_contents("u3");
    $clearlist4 = file_get_contents("u4");
    $clearlist5 = file_get_contents("u5");
    $clearlist6 = file_get_contents("u6");
    $clearlist7 = file_get_contents("u7");
    $clearlist8 = file_get_contents("u8");
    ...
    bot('sendMessage', ['chat_id' => $chat_id, 'text' => 'Done clear all list 🐊', 'reply_markup' => json_encode(['inline_keyboard' => [[['text' => "->", 'callback_data' => "#Back"]]]])]);
    }
...
if ($text == "- To Channel All") {
    ...
    $channel = file_get_contents("type");
    $channel1 = file_get_contents("type1");
    $channel2 = file_get_contents("type2");
    $channel3 = file_get_contents("type3");
    $channel4 = file_get_contents("type4");
    $channel5 = file_get_contents("type5");
    $channel6 = file_get_contents("type6");
    $channel7 = file_get_contents("type7");
    $channel8 = file_get_contents("type8");
    
    file_put_contents("type", "c");
    file_put_contents("type1", "c");
    ...
    file_put_contents("type8", "c");
        ...
        bot('sendMessage', ['chat_id' => $chat_id, 'text' => 'Done add type (c) to all accounts ✅', 'reply_markup' => json_encode(['inline_keyboard' => [[['text' => "->", 'callback_data' => "#Back"]],]])]);
        }
if ($text == "- Run All") {
...
$info["loop1"] = 0;
file_put_contents('R1.json', json_encode($info));
}
...
$info["loop2"] = 0;
file_put_contents('R2.json', json_encode($info));
}
...
$info["loop3"] = 0;
file_put_contents('R3.json', json_encode($info));
}
...
$info["loop4"] = 0;
file_put_contents('R4.json', json_encode($info));
}
...
$info["loop5"] = 0;
file_put_contents('R5.json', json_encode($info));   
}
...
$info["loop6"] = 0;
file_put_contents('R6.json', json_encode($info));
}
...
shell_exec("pm2 start 7a.php");
$info["loop7"] = 0;
file_put_contents('R7.json', json_encode($info));
}
...
shell_exec("pm2 start 8a.php");
$info["loop8"] = 0;
file_put_contents('R8.json', json_encode($info));
}
...
$num8 = $info["num8"];
if ($data == "#Back") {
 bot('editMessagetext',['chat_id'=>$chat_id2,'message_id'=>$message_id,'text' => "Welcome , Developer ",
'reply_markup'=>json_encode(['inline_keyboard'=>[
